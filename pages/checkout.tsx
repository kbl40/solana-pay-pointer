  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      try {
        ...
        const singatureInfo = await findTransactionSignature(connection, reference)
        ...
        if (e instanceof ValidateTransactionSignatureError) {
          console.error('Error trying to validate signature', e)
          return
        }
  ...
  if (!publicKey) {
    return (
      <div className='flex flex-col gap-8 items-center'>
        ...
        <WalletMultiButton />
        <p>You need to connect your wallet to make transaction</p>
      </div>
    )
  }
